...
function ActionButton({ action, game, act, active = false, children }: { action: Action; game: Game; act: (action: Action) => void; active?: boolean; children: ReactNode }) {
  ...
  return <button className={`control-button ${active ? "active-effect" : ""}`} disabled={game.phase !== "running" || remaining > 0} onClick={() => act(action)} aria-label={ACTION_NAMES[action]}><span>{children}</span>{remaining > 0 && <small>{active ? "ACTIVE" : cooldownLabel}</small>}</button>;
}
...
export default function Home() {
  ...
  const alert = useMemo(() => {
    ...
    if (game.oxygenDelivery < 85) return "Oxygen delivery is falling — check heart rate and hydration.";
    ...
    if (warning === "temperature" && DEFAULT_CONFIG.humidity >= 50 && game.sweatLevel >= 3) return "Humidity limits evaporation — more sweat may not add cooling. Reduce pace.";
    ...
    if (game.reserve < 25) return "Exercise reserve is running low — reduce pace to recover.";
  ...
  return <main className="app-shell">
    ...
    <div className="game-grid">
      ...
      <section className="race-panel" aria-label="Race view">
        ...
        <div className="scene"><div className="sun" /><div className="horizon" /><div className="road-line one" /><div className="road-line two" />
          ...
          {game.phase === "ready" && <button className="start-race" onClick={() => setSession((current) => ({ ...current, game: startGame(current.game) }))}>START RACE <b>→</b></button>}<div className="finish-flag"><span>40K</span></div>
        </div>
        ...
        <div className="secondary-readouts"><span>Exercise reserve <b>{game.reserve.toFixed(0)}%</b></span><span>Oxygen delivery <b>{game.oxygenDelivery.toFixed(0)}%</b></span><span>Hydration balance <b>{hydrationDisplay >= 0 ? "+" : ""}{hydrationDisplay.toFixed(1)}%</b></span><span>Liver glycogen <b>{game.glycogen.toFixed(0)} g</b></span><span>Muscle glycogen <b>{game.muscleGlycogen.toFixed(0)} g</b></span><span>Sweat level <b>{game.sweatLevel}/3</b></span></div>
      </section>
